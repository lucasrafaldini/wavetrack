...
function renderHistory() {
 ...
 <span>
 <strong>${formatDuration(day.total_hours * 3600)}</strong> total
 </span>
 </div>
 </div>
...
function rowHTML(device) {
 ...
 : `<button class="btn btn-primary" onclick="openEditModal('${device.mac_address}')" style="background:#ffc107;border:none;"> Editar</button>`}
 </td>
 </tr>
 `;
}
...
async function cleanupInactive() {
 if (!confirm(' Tem certeza? Isso irá remover todos os dispositivos inativos sem cadastro de colaborador.\n\nEsta ação não pode ser desfeita!')) {
 return;
 }
